...
@dataclass
class EntropyPolyApproximation1Deg3:
    epsilon: float

    def calc_approximation(self, x: float) -> float:
        return 7/4/(self.epsilon ** 2) * x * x * x - 4/self.epsilon*x*x + (137/60 - math.log(self.epsilon)) * x
    ...
    def calc_approximation_on_vector(self, x_sum: float, y_sum: float, x_squared_sum: float, y_squared_sum: float,
        ...
        term1 = (137/60 - math.log(self.epsilon)) / 2 * (x_sum + y_sum)
        term2 = (- 4/self.epsilon / 4) * (x_squared_sum + y_squared_sum + 2 * xy)
        term3 = (7/4/(self.epsilon ** 2) / 8) * (x_tripled_sum + y_tripled_sum + 3*xxy + 3*xyy)
...
if __name__ == '__main__':
    EntropyPolyApproximation1Deg3(0.0005).draw_approximation(join_create_dir('..', RESULTS_DIR_PATH, 'entropy_poly_approximation1_deg_3'))
