...
def generators() -> Iterable[Tuple[str, Callable[[], float]]]:
    np.random.seed(200)
    yield 'Uniform (0-0.1)', lambda: np.random.uniform(0, 0.1)
    ...
    yield 'Exponential scale=0.02', lambda: np.random.exponential(scale=0.02)
...
def main() -> None:
    ...
    for (generator_name1, generator1), (generator_name2, generator2) in combinations_with_repetitions(generators()):
        ...
        for top_n in range(vec_length):
            ...
            print(f'{top_n},{generator_name1},{generator_name2},{average_entropy},{vec_length},'
                  f'{lower_bound_vec1},{upper_bound_vec1},{lower_bound_vec2},{upper_bound_vec2},{entropy1},{entropy2}')
